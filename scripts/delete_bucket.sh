...
#
# This script deletes a bucket in Ozone
#
...
if [ "$#" -ne 1 ]; then
  ...
  echo "deletes a bucket in ozone server" 
  exit 1
fi
...
HTTP_STATUS=$(curl $VERBOSE -X $VERB --write-out '%{http_code}' -H "$AUTH" -H $HOST $URL)
if [ -n "$OZONE_VERBOSE" ]; then 
	HTTP_STATUS=$(expr $HTTP_STATUS + 0)
fi
echo -e "Command:Delete Bucket:\t\t$VERB $URL --> return status: $HTTP_STATUS"
