...
#
if [ "$#" -ne 3 ]; then
  echo "Usage: $0 <volume> <bucket> <file>" >&2
...
hash=`md5 -q $1`
CONTENT_HASH="Content-MD5:$hash"

HTTP_STATUS=$(curl $VERBOSE -X $VERB -w '%{http_code}' --data-binary @$3 -H "$AUTH" -H "$OZONE_VERSION" -H "$OZONE_USER" -H "$TYPE" -H "$CONTENT_HASH" -H"$DATE" $URL)
if [ -n "$OZONE_VERBOSE" ]; then
	HTTP_STATUS=$(expr $HTTP_STATUS + 0)
fi
echo -e "Command:put key:\t\t$VERB $URL --> return status: $HTTP_STATUS"
